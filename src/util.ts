...
import type { SFTPLoaderPathInfo } from './config'
import { Buffer } from 'node:buffer'
import { createHash } from 'node:crypto'

export function formatPath(output: IImgInfo, userConfig: SFTPLoaderPathInfo): SFTPLoaderPathInfo {
  // 获取日期
  const date = new Date()
  ...
  // 未格式化路径
  const pathInfo: SFTPLoaderPathInfo = {
    path: userConfig.path,
    uploadPath: userConfig.uploadPath
  }

  // 替换后的路径
  const formatPath: SFTPLoaderPathInfo = {
    path: '',
    uploadPath: ''
  }
  ...
  for (const key in pathInfo) {
    ...
    const typedKey = key as keyof SFTPLoaderPathInfo
